...
//
// Vive no repo do 102041 (mls-102041/scripts/publish), mas opera no monorepo
// mls-base que o contém: o staging (.publishStudio) e o tsc rodam na raiz.
//
// Credenciais: mls-102041/servers/s3.conf (gitignored; ver s3.conf.example) ou env.
//
// Uso (a partir da raiz do mls-base):
//   node mls-102041/scripts/publish/publishStudio.mjs list
//   node mls-102041/scripts/publish/publishStudio.mjs rollback <datetime14> [--lang en]
//   node mls-102041/scripts/publish/publishStudio.mjs publish [--langs en,pt] [--copy-assets]
//        [--skip-compile] [--dry-run] [--project 102041] [--theme Default]
// ou, de dentro de mls-102041: pnpm publish:studio -- <args>  (ver package.json)
...
// mls-102041/scripts/publish/ -> mls-102041 (PROJECT_DIR) -> mls-base (ROOT)
const PROJECT_DIR = resolve(dirname(fileURLToPath(import.meta.url)), '..', '..');
const ROOT = resolve(PROJECT_DIR, '..');
if (!existsSync(join(ROOT, 'tsconfig.json'))) {
  throw new Error(`não achei a raiz do mls-base em ${ROOT} (o script espera viver em <mls-base>/mls-102041/scripts/publish)`);
}
...
// ── config (mls-102041/servers/s3.conf + env) ────────────────────────────────
// Primeiro o conf do próprio projeto; <mls-base>/servers/s3.conf fica como
// fallback para máquinas que ainda têm o arquivo no lugar antigo.
const CONF_PATHS = [join(PROJECT_DIR, 'servers', 's3.conf'), join(ROOT, 'servers', 's3.conf')];

function loadConf() {
  const conf = {};
  const path = CONF_PATHS.find((p) => existsSync(p));
  if (path) {
...
function assertS3Conf(conf) {
  ...
  if (missing.length) {
    throw new Error(
      `${relative(ROOT, CONF_PATHS[0]).replace(/\\/g, '/')} incompleto ou ausente (faltam: ${missing.join(', ')}). ` +
      'Copie mls-102041/servers/s3.conf.example.',
    );
  }
}
